...
Scenario: Verify that dawg pound STB reservation list contains STBs added to dawg house 
	When I send GET request to 'get STB reservation list' service 
	Then I should receive status code 200
...
Scenario: Reserve an STB device on dawg pound via POST request
    When I send POST request to 'reserve STB' service with following details
    | id        | token       |
    | poundrest | reservetest |
    Then I should receive status code 200
    When I send GET request to 'get STB reservation token' service with same reservation token
...
Scenario: Get STB reservation token from dawg pound via GET request with invalid reservation token
	Given I send GET request to 'get STB reservation token' service with invalid token
...
Scenario: Check if an STB device is reserved on dawg pound via GET request with valid device id
    Given I send POST request to 'reserve STB' service with following details
    | id        | token       |
    | poundrest | reservetest |
    Then I should receive status code 200   
    When I send GET request to 'check if STB is reserved' service with valid device id
    Then I verify that the response contains expected reservation token

Scenario: Check if an STB device is reserved on dawg pound via GET request with invalid device id
    When I send GET request to 'check if STB is reserved' service with invalid device id
...
Scenario Outline: Verify the response of STB reservation on dawg pound with invalid device id
    When I send POST request to 'reserve STB' service with invalid <param>
    Then I should receive status code 200   
...
Scenario: Reserve an STB device which is already reserved with different reservation token on dawg pound 
    ...
    | poundrest | reservetest |
    Then I should receive status code 200   
    When I send POST request to 'reserve STB' service for same STB device with different reservation token
...
Scenario: Reserve an STB device on dawg pound which is non reserved via POST request
    Given I send POST request to 'unreserve STB' service with following details
    | id        |
    | poundrest |
    Then I should receive status code 200   
    When I send POST request to 'reserve STB' service for same STB device with different reservation token
    And I verify that the response contains same reservation token
...
Scenario: Unreserve an STB device which is already reserved via POST request with valid device id
    ...
    | poundrest | reservetest |
    Then I should receive status code 200 
    When I send POST request to 'unreserve STB' service for same STB device
    Then I should receive status code 200   
    And I verify that the response contains same reservation token
    
Scenario: Unreserve an STB device which is already reserved via POST request with invalid device id
    Given I send POST request to 'unreserve STB' service for an STB which is already reserved with invalid device id
