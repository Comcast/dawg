...
Background: 
	Given I send POST request to 'add an STB model' service with following details
...
Scenario: Retrieve STB model details from dawg house via GET request
	When I send GET request to 'get STB model by id' service with valid model id
	Then I should receive status code 200
...
Scenario: Add a new STB model to dawg house via POST request with specified properties
	When I send POST request to 'add STB model' service with following properties
...
Scenario: Update fields for an STB model already present in the dawg house via POST request
	When I send POST request to 'add STB model' service with following properties
	| model id | name  | Capabilites | family   |
	| DTA30    | DTA30 | VOD         | newfamily|
	Then I should receive status code 200
...
Scenario: Remove an existing STB model from dawg house via DELETE request with valid model id
	When I send DELETE request to 'delete STB model' with valid STB device id
	Then I should receive status code 200
...
Scenario Outline: verify that STB Model fields gets reflected in STB device on sending GET request to 'assign models' service
	When I send PUT request to 'add an STB' service with STB device id <id>, mac address <mac>, STB model name <model>, model family <Family> and model capabilities <Capabilities> 
