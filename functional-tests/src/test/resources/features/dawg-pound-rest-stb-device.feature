@dawg_rest @dawg_pound
Feature: REST verification STB device reservation from Dawg Pound
...
Scenario: Get STB reservation token from dawg pound via GET request with valid reservation token
...
Scenario: Get STB reservation token from dawg pound via GET request with invalid reservation token
	Given I send GET request to 'get STB reservation token' with invalid reservation token
...
Scenario: Reserve an STB device which is already reserved with different reservation token on dawg pound 
    Given I send POST request to 'reserve STB' politely with following details
    | id        | token       |
    | poundrest | reservetest |
    Then I should receive status code 200   
